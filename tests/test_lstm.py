...
def test_train_returns_artifacts_with_fitted_scaler() -> None:
    df = _frame(120)
    art = train_lstm(df, lookback=10, epochs=2)
    # Scaler must be fitted (has learned means).
...
def test_predictions_align_to_last_day_of_windows() -> None:
    ...
    art = train_lstm(train, lookback=10, epochs=2)
    preds, y_true = predict_lstm(art, test)
    # With lookback=10 on 60 test rows: 60 - 10 + 1 = 51 predictions.
    assert len(preds) == 51
    assert len(y_true) == 51
    # Predictions align to the LAST (lookback-1) rows onward.
...
def test_train_rejects_too_small_slice() -> None:
    ...
    with pytest.raises(ValueError, match="too small"):
        train_lstm(tiny, lookback=60, epochs=2)
